...
//	**dataset_dir
//	**result_dir.
//	D:\\Johnny_Galecki_after_filtering\\src D:\\Johnny_Galecki_after_filtering
//	*/
//	if (argc < 3) {
...
int main(int argc, char* argv[]) {
	...
	*/
	...
	if (exists("standard_image_info_feature_test.txt")) {
		ifstream in_st_fea;
		in_st_fea.open("standard_image_info_feature_test.txt", ios::in);
		string tmp_str;
		in_st_fea >> tmp_str;
		if (tmp_str == argv[2]) {
			have_done = true;
	...
	if (!have_done) {
		...
		save_st_fea.open("standard_image_info_feature_test.txt", ios::out);
	...
	int i = 0;
	if (exists("log_of_test.txt")) {
		ifstream ifile;
		ifile.open("log_of_test.txt", ios::in);
		ifile >> i;
		ifile.close();
	}
	...
	ofstream ofile;
	ofile.open("log_of_test.txt", ios::out);
